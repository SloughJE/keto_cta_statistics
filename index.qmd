...
- **Participants:** 100 adults metabolically healthy adults meeting *lean mass hyper-responder* (LMHR) 
  - profile: keto ≥2 yrs, BMI <25, LDL-C ≥190, HDL-C ≥60, TG ≤80  
<br>

::: {.fragment}
  
- **Findings:** No association between ApoB or LDL-C (baseline or change) and plaque progression  
- **Predictors:** Baseline plaque burden (CAC, NCPV, TPS, PAV) predicted ΔNCPV  
- **Stats:** Univariable models used on primary and secondary outcomes; Bayesian analysis favored null for ApoB (BF 6–10 in favor of no effect)  
<br>
...
["Many authors and pharmaceutical clinical trialists make the mistake of analyzing change from baseline instead of making the raw follow-up measurements the primary outcomes, covariate-adjusted for baseline."](https://www.fharrell.com/post/errmed/#change-from-baseline) - Frank E Harrell Jr, PhD, Professor of Biostatistics, Vanderbilt University School of Medicine 
...
- Obscures clinically meaningful patterns  
- Inflates error, reducing model precision  
- Introduces Mathematical Coupling  
...
df <- data.frame(
  Subject = c("A", "B", "C"),
  ApoB_mg_dL = c(120, 185, 250),
  Delta_NCPV_mm3 = c(20, 20, 20)
)
...
df <- data.frame(
  Subject = c("A", "B", "C"),
  ApoB_mg_dL = c(120, 185, 250),
  NCPV_0_mm3 = c(20, 76, 130),
  NCPV_1_mm3 = c(40, 96, 150),
  Delta_NCPV_mm3 = c(20, 20, 20)
)
...
<br>
<ul style="margin-top: 0; margin-bottom: 0; line-height: 1.2; padding-left: 1.5em; font-size: 90%;">
  <li>All three subjects had the same plaque progression, but started and ended at very different levels.</li>
  <li>ApoB increases across subjects — but ΔNCPV doesn’t reflect this.</li>
  <li>If you model ΔNCPV ~ ApoB, the slope will be zero, even though ApoB clearly tracks with absolute plaque burden.</li>
</ul>
...
- Simulated data (N = 100) approximates ApoB and $\text{NCPV}_0$ based on summary stats from study  
- We compare model: **ΔNCPV ~ ApoB** with **NCPV₁ ~ NCPV₀ + ApoB**
...
n <- 100
mean_apob <- 185; sd_apob <- 50.8
mean_ncpv0 <- 75.9; sd_ncpv0 <- 88.3

apob <- rnorm(n, mean_apob, sd_apob)
ncpv_0 <- rnorm(n, mean_ncpv0, sd_ncpv0)

ncpv_1 <- 0.125 * apob + 0.6 * ncpv_0 + rnorm(n, 0, 15)
delta_ncpv <- ncpv_1 - ncpv_0

model1 <- lm(delta_ncpv ~ apob)
model2 <- lm(ncpv_1 ~ ncpv_0 + apob)

# Extract key stats
tbl <- data.frame(
  Model = c("ΔNCPV ~ ApoB", "NCPV1 ~ NCPV0 + ApoB"),
  `Residual SE` = c(round(summary(model1)$sigma, 2), round(summary(model2)$sigma, 2)),
  `SE (ApoB)` = c(round(summary(model1)$coefficients["apob", "Std. Error"], 3),
                  round(summary(model2)$coefficients["apob", "Std. Error"], 3)),
  `p-value (ApoB)` = c(
    signif(summary(model1)$coefficients["apob", "Pr(>|t|)"], 2),
    signif(summary(model2)$coefficients["apob", "Pr(>|t|)"], 2)
  ),
  `R²` = c(round(summary(model1)$r.squared, 2), round(summary(model2)$r.squared, 2))
)
...
<ul style="margin-top: 0; margin-bottom: 0; line-height: 1.2; font-size: 80%;">
  <li>Both models test whether ApoB is associated with plaque progression</li>
  <li>Modeling follow-up plaque while adjusting for baseline gives a clearer answer</li>
  <li>Change scores often reduce power and obscure relationships</li>
</ul>
...
resid_df <- data.frame(
  Residual = c(residuals(model1), residuals(model2)),
  Model = factor(rep(c("Delta NCPV ~ ApoB", "NCPV1 ~ NCPV0 + ApoB"), each = n))
)

plt <- ggplot(resid_df, aes(x = Residual, color = Model, fill = Model)) +
  geom_density(alpha = 0.3, adjust = 1.2) +
  labs(
    title = "Residual Distributions: ΔNCPV vs Baseline-Adjusted Model",
    x = "Residual",
    y = "Density"
  ) +
...
ggplotly(plt, width = 500, height = 300) %>%
   layout(
    legend = list(orientation = "h", x = 0.5, xanchor = "center", y = -0.2),
    xaxis = list(title = list(text = "Residual", standoff = 5))  # Reduce distance from plot
...
<div style="font-size: 85%; line-height: 1.3;">
...
<ul style="margin-top: 0; margin-bottom: 0; line-height: 0.9; font-size: 80%; padding-left: 1.5em;">
  <li>$\rho = \operatorname{Cor}(\text{NCPV}_0,\ \text{NCPV}_1)$</li>
  <li>$\sigma_0$, $\sigma_1$ = SDs at baseline and follow-up</li>
</ul>
...
<ul style="margin-top: 0; margin-bottom: 0; line-height: 0.9; font-size: 90%; padding-left: 1.5em;">
  <li>**correlation** between baseline and follow-up ($\rho$)</li>
  <li>**relative spread** of the two time points</li>
</ul>
...
<ul style="margin-top: 0; margin-bottom: 0; line-height: 0.9; font-size: 90%; padding-left: 1.5em;">
  <li>Almost all participants had increased NCPV, so $\rho$ is plausibly high</li>
  <li>if increases were heterogeneous, $\sigma_1$ is also likely > $\sigma_0$</li>
</ul>
...
["Many authors and pharmaceutical clinical trialists make the mistake of analyzing change from baseline instead of making the raw follow-up measurements the primary outcomes, covariate-adjusted for baseline."](https://www.fharrell.com/post/errmed/#change-from-baseline) - Frank E Harrell Jr, PhD, Professor of Biostatistics
:::
...
So they did include three multivariable models **multivariable models**:
...
#### Simulated Prior Scale Sensitivity Analysis on ΔNCPV ~ ApoB Model
...
1. **ΔNCPV as Outcome**  
   Regressed (NCPV₁ − NCPV₀) on baseline metrics <br>
   Widely discouraged in statistical literature → Mathematical coupling, inflated error, and clinically ambiguous interpretation
...
3. **Bayesian Inference**  
   ...
   Applied atop unadjusted, structurally weak models → **amplifies conclusions**
:::
::: {.fragment}

4. **Bayesian Prior Choice (rscale = 0.8)**  
...
5. **Bayes Factor Interpretation**  
   Claimed ApoB no association is "6–10× more likely...true"<br>
...
6. **Headline claim**  
   ...
   Overstates evidence → undermined by methodological limitations, unadjusted modeling, and incomplete model reporting
...
<ul style="margin-top: 0; margin-bottom: 0; line-height: 0.9; font-size: 80%; padding-left: 1.5em;">
  ...
  <li>Rationale for subgroup thresholds not explained.</li>
</ul>
...
<ul style="margin-top: 0; margin-bottom: 0; font-size: 85%; line-height: 1; padding-left: 1.5em;">
  <li>Normality: Shapiro-Wilk test, Kolmogorov–Smirnov test, Q-Q plots (visual / quantitative)</li>
  <li>Homoscedasticity: Breusch–Pagan test, White test</li>
...
In that citation [[7]](https://pubs.rsna.org/doi/abs/10.1148/radiol.2241011191?journalCode=radiology), Yoon et al. did perform t-tests on change scores, but their main conclusions relied on a random-effects repeated-measures **multivariable** regression model of follow-up CAC, **adjusted for baseline plaque** and clinical risk factors.
...
#### [https://github.com/SloughJE/keto_cta_statistics](https://github.com/SloughJE/keto_cta_statistics)
